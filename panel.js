...
function normalizeSpamConfig(raw={}){const level=Math.max(0,Math.min(3,Number(raw.level)||0)),preset=[{}, {capsPercent:85,capsMinLetters:15,maxEmotes:14,maxLength:500,maxSymbols:28,maxRepeats:8,oneManMessages:9,oneManWindow:8}, {capsPercent:75,capsMinLetters:11,maxEmotes:10,maxLength:380,maxSymbols:20,maxRepeats:6,oneManMessages:7,oneManWindow:8}, {capsPercent:65,capsMinLetters:8,maxEmotes:7,maxLength:280,maxSymbols:14,maxRepeats:4,oneManMessages:5,oneManWindow:7}][level];return {...SPAM_DEFAULTS,...raw,level,filters:{...SPAM_DEFAULTS.filters,...(raw.filters||{})},limits:{...SPAM_DEFAULTS.limits,...preset,...(raw.limits||{})},allowedDomains:(Array.isArray(raw.allowedDomains)?raw.allowedDomains:String(raw.allowedDomains||'').split(/[\s,;]+/)).map(x=>String(x).trim().toLowerCase()).filter(Boolean).slice(0,50)}}
