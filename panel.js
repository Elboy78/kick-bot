...
app.get('/api/admin/memes', requireAuth, requireTenant, async (req,res) => {
  try {
    ...
    res.json({data:{config,overlayUrl:`${base}/o/${token.token}/memes.html`,viewerUrl:`${base}/memes/${tm.slug}?token=${testToken}`,points:{amount:Number(points.meme_points_amount||5),interval:Number(points.meme_interval_minutes||10),starting:Number(points.meme_starting_points||100)},leaderboard}});
  } catch(e) { res.status(500).json({error:e.message}); }
});
...
app.post('/api/public/memes/:streamer/submit',async(req,res)=>{try{
  ...
  let ttsUrl=null;if(wantsTts){const audio=await tenant.runWithStreamer(s,()=>generateTTSAudio(text));if(!audio)return res.status(503).json({error:'La voix IA ElevenLabs n’est pas configurée'});const audioFile=`${s.id}-${Date.now()}-${crypto.randomBytes(5).toString('hex')}.mp3`;await fs.promises.writeFile(path.join(memeTempDir,audioFile),Buffer.from(audio,'base64'));ttsUrl=`/meme-media/${audioFile}`}
  ...
  const row=await db.createMemeSubmission(s.id,username,text,mediaUrl,status,{mediaType,ttsUrl});memeCooldowns.set(cooldownKey,Date.now()+cfg.cooldown*1000);
  if(instant){const payload={username,text,mediaUrl,mediaType,ttsUrl,duration:mediaType==='video'?Math.min(cfg.maxVideoDuration,videoDuration):cfg.duration,size:cfg.size,position:cfg.position,launchSound:cfg.launchSound,launchSoundVolume:cfg.launchSoundVolume,volume:0,at:new Date().toISOString()};await db.createMemeEvent(s.id,payload);tm.emit('meme-overlay-event',payload)}
